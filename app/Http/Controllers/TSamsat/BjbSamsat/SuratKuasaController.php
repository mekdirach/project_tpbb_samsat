...
class SuratKuasaController extends Controller
{
    public function index() {
        ...
        $template = MpMasterSuratKuasa::where('status', 1)->where('kode_suratkuasa', 's')->get();
        $tJson = json_decode($template);
        return view('t-samsat.bjb-tsamsat.surat-kuasa.index', compact('pemda', 'tJson'));
    }

    public function detail(Request $request) {
        $record = MpCustomer::where('nomor_polisi', $request->nopol)
        ->where('tahun_pajak', $request->tahunPajak)
        ->where('kode_provinsi', $request->pemdaID)
        ->where('rekening_external', $request->accountNumber)
        ->first();
