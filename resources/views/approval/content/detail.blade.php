    ...
    <div class="card-body">
        <div class="row justify-content-start d-flex">
            ...
            <div class="col-md-auto ml-3">
                <div class="row m-0">
                    <h6> : {{ $record->pbb_c_id }}</h6>
                </div>
                <div class="row m-0">
                    <p> : {{ $record->pbb_c_nop }}</p>
                </div>
                <div class="row m-0">
                    <p> : {{ $record->pbb_c_tahun }}</p>
                </div>
            </div>
        </div>
        <div class="row mt-3">
                ...
                <div class="card-body" style="border-radius: 5px; background: rgba(224, 229, 232, 0.75);">
                    <div class="row">
                        ...
                        <div class="col-md-auto">
                            ...
                            <div class="row">
                                <p>{{ $record->pbb_c_pajak_tgl_rencana_bayar }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body" style="border-radius: 5px; background: rgba(224, 229, 232, 0.75);">
                    <div class="row">
                        ...
                        <div class="col-md-auto">
                            ...
                            <div class="row">
                                <p>Rp {{ number_format($record->pbb_c_amount) }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-3 mt">
                        ...
                        <div class="row"><p>{{ $record->pbb_c_nop_name}}</p></div>

                        <div class="row"><h6>Nama Pemda</h6></div>
                        <div class="row"><p>{{ $record->mp_p_nama}}</p></div>

                        <div class="row"><h6>Luas Tanah</h6></div>
                        <div class="row"><p>{{ $record->pbb_c_luas_tanah}} m<sup>2</sup></p></div>

                        <div class="row"><h6>Luas Bangunan</h6></div>
                        <div class="row"><p>{{ $record->pbb_c_luas_bangunan}} m<sup>2</sup></p></div>

                        <div class="row"><h6>Lokasi</h6></div>
                        <div class="row"><p>{{ $record->pbb_c_lokasi}}</p></div>
                    </div>
                    <div class="col-3">
                        <div class="row"><h6>Provinsi</h6></div>
                        <div class="row"><p>{{ $record->mp_mp_nama}}</p></div>

                        <div class="row"><h6>Kota/Kabupaten</h6></div>
                        <div class="row"><p>{{ $record->mp_mkk_nama}}</p></div>

                        <div class="row"><h6>Kecamatan</h6></div>
                        <div class="row"><p>{{ $record->pbb_c_kecamatan}}</p></div>

                        <div class="row"><h6>Kelurahan</h6></div>
                        <div class="row"><p>{{ $record->pbb_c_kelurahan}}</p></div>

                        <div class="row"><h6>Jenis Registrasi</h6></div>
                        <div class="row"><p>{{ $record->pbb_c_reg_type}}</p></div>
                    </div>
                    <div class="col-3">
                        <div class="row"><h6>Tanggal Awal Blokir</h6></div>
                        <div class="row"><p>{{ $record->pbb_c_autoblokir_awal_tgl}}</p></div>

                        <div class="row"><h6>Tanggal Akhir Blokir</h6></div>
                        <div class="row"><p>{{ $record->pbb_c_autoblokir_akhir_tgl}}</p></div>

                        <div class="row"><h6>Nominal Pembayaran</h6></div>
                        <div class="row"><p>Rp {{ number_format($record->pbb_c_amount)}}</p></div>

                        <div class="row"><h6>Biaya Admin</h6></div>
                        <div class="row"><p>Rp {{ number_format($record->mp_p_fee)}}</p></div>

                        <div class="row"><h6>Diskon Pembayaran</h6></div>
                        <div class="row"><p>Rp {{ number_format($record->pbb_c_diskon)}}</p></div>
                    </div>
                    <div class="col-3">
                        <div class="row"><h6>Layanan Notifikasi</h6></div>
                        <div class="row"><p>{{ $record->status_inputan}}</p></div>

                        <div class="row"><h6>Status Notifikasi</h6></div>
                        <div class="row"><p>{{ $record->status_inputan}}</p></div>

                        <div class="row"><h6>Nomor Ponsel</h6></div>
                        <div class="row"><p>{{ $record->pbb_c_phone}}</p></div>

                        <div class="row"><h6>Alamat Email</h6></div>
                        <div class="row"><p>{{ $record->pbb_c_mail}}</p></div>
                ...
                <table class="table table-borderless">
                    ...
                    <tbody>
                        @if ($record->pbb_c_reg_type == 'BERKALA')
                            @foreach ($recordType as $item)
                            <tr>
                                <th>{{ $item->mp_pbb_ab_tgl_blokir }}</th>
                                <th>Rp {{ number_format($item->mp_pbb_ab_amount) }}</th>
                                <th>{{ $item->mp_pbb_ab_flag }}</th>
                                <th>{{ $item->mp_pbb_ab_message }}</th>
                            </tr>
                            @endforeach
                        @else
                            @foreach ($recordType as $item)
                            <tr>
                                <th>{{ $item->mp_pbb_as_tgl_blokir }}</th>
                                <th>Rp {{ number_format($item->mp_pbb_as_amount) }}</th>
                                <th>{{ $item->mp_pbb_as_flag }}</th>
                                <th>{{ $item->mp_pbb_as_message }}</th>
                            </tr>
                            @endforeach
                        @endif
                    </tbody>
