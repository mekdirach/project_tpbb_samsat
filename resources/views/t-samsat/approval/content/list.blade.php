    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#btnCheckUser').click(function (e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('approval-samsat.check-user') }}",
                    data: {
        ...
        function loadData() {
            $('.datatables-demos').dataTable({
                ...
                ajax: {
                    url: "{{ route('approval-samsat.list') }}",
                    type: 'POST',
                    data: {
                        _token: $('meta[name="csrf-token"]').attr('content'),
                        branch: $('#branch').val(),
        ...
        function setApprove(nopol, tahun, label) {
            ...
            if (approval_type == 1) {
                route = "{{ route('approval-samsat.update-approve-registrasi') }}"
            }else if (approval_type == 2){
                route = "{{ route('approval-samsat.update-approve-pembatalan') }}"
            }else if (approval_type == 3){
                route = "{{ route('approval-samsat.update-approve-pembatalan-aro') }}"
            }
        ...
        function setReject(nopol, tahun, label) {
            ...
            if (tCheckedData.length > 0) {
                $.ajax({
                    url: "{{ route('approval-samsat.update-reject') }}",
                    type: 'post',
                    data: {
                        _token: $('meta[name="csrf-token"]').attr('content'),
                        nopol: tCheckedData
